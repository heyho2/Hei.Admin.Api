...
namespace Hei.Admin.IRepository
{
    ...
    public interface IBaseRepository<T> : IService where T : BaseModel
    {
        ...
        /// <returns></returns>
        List<TResult> GetListPaging<TResult>(Expression<Func<T, bool>> whereLambda, Expression<Func<T, TResult>> selector, out int total, int pageIndex, int pageSize, string order, string sort);
    }
}